...
class AFTLocal(nn.Module):
  def __init__(self, d_model: int, max_seqlen: int, d_hidden: int, local_window_size: int, activation=F.sigmoid, bias: bool=False) -> None:
    super().__init__()
    self.d_model = d_model
    self.d_hidden = d_hidden
    ...
    self.w_o = nn.Linear(d_hidden, d_model, bias=bias)
    self.W_bias = nn.Parameter(torch.Tensor(max_seqlen, max_seqlen), requires_grad=True)
    self.local_mask = nn.Parameter(self.create_local_mask(max_seqlen, local_window_size), requires_grad=False)
    nn.init.xavier_uniform_(self.W_bias)
  @staticmethod
  ...
  def forward(self, q, k, v, mask=None):
    ...
    K = self.w_k(k)
    V = self.w_v(v)
    pos_bias = self.W_bias[:Ti, :Tj] * self.local_mask[:Ti, :Tj]
